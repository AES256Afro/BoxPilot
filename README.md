...
| Area | Status in `0.41.0` | Capability |
| --- | --- | --- |
...
| Repair Center | Live foundation plus two exact repairs | Checks Node.js, state storage, APT metadata, `smartmontools`, the helper, Docker, libvirt, Tailscale, and DNS port availability without returning peer details or raw command output. The fixed `smartmontools` repair and separately named metadata-only APT refresh use immutable plans, separate staging, password approval, durable steps, exact state revalidation, and post-operation verification. It also builds a read-only secret-free recovery readiness view and ordered exportable runbook. |
...
| Disaster recovery kit | Authenticated read-only export | Correlates sanitized job, local controller-backup, independently protected controller-snapshot, application-backup, protected-VM-backup, router-checkpoint, migration, fleet, DNS, and prerequisite evidence. Local-only controller evidence remains an operator check; encrypted independent repository plus exact restored database evidence becomes verified. JSON and Markdown downloads remain evidence only: no database, application data, configuration file, backup payload, credential, or mutation is included. |
| Restricted helper | Live typed operations | Uses a versioned, allowlisted protocol over a local Unix socket for fixed prerequisite, inventory, controller backup, independent protection and retention, curated application, migration, and VM workflows. Keel discovery is parameter-free and returns only bounded installation, persistence, listener, health, and risk evidence. Controller protection accepts only two UUIDs, two recorded hashes, an exact size, and a destination revision. Controller retention accepts only a server-generated UUID, fixed repository and destination evidence, the complete snapshot-set revision, and one to 100 exact sorted snapshot ids selected by server policy. No browser password, path, repository, selector, command, endpoint, retention rule, forget option, or prune input exists. |
...
| Keel Notes adapter | Read-only discovery and immutable planning | Inspects bounded native user-service and Docker evidence, fixed port 3000 exposure, persistent-data signals, and exact Keel health identity, then binds a non-executable plan to the fixed `v1.2.5` Linux x64 release metadata. It exposes no username, private path, container id, environment, database content, or secret. Download, local byte verification, extraction, installation, adoption, start, claim, backup, restore, import, exposure, and staging are locked. |
...
| Backup engine | Controller plus two application adapters | Creates a no-downtime WAL-aware BoxPilot SQLite snapshot and immutable local Uptime Kuma and Pi-hole archives. The controller adapter can then copy one verified local snapshot to its separate encrypted mounted-restic repository, perform a complete repository read, restore the exact snapshot, rehash both files, and reopen the database before reporting independent protection. A separate fixed controller-retention workflow preserves the minimum-copy, minimum-age, restore, and active-operation floors before forgetting exact old snapshot references. Application artifacts remain local-only. |
...
- Backup schedules, independent application-backup destinations, controller remote/cloud adapters, browser download, automatic controller production restore, restic prune and space reclamation, configurable controller retention beyond the fixed evidence-gated policy, remote restic/cloud backends, Keel Notes export, SSH source discovery or transport, general application-aware volume/database capture, staged-workload activation, or migration cutover
...
### Keel Notes discovery and planning mockup

![BoxPilot read-only Keel Notes discovery and immutable release plan](docs/screenshots/keel-plan-mock.png)

This explicitly disclosed `0.41.0` mock shows an absent-install result from bounded parameter-free native and Docker discovery, the fixed Keel release identity, GitHub metadata match, absent local-byte verification, recovery boundaries, and the permanent execution blocker. No `.env`, database, secret, private path, container id, image, unit content, or mount source was included. No Keel asset was downloaded, no local digest was computed, no application tree or account was created, and no service, container, port, or data was changed for the capture.
...
- [Keel Notes read-only discovery, exact-release planning, and execution gates](docs/KEEL.md)
...
Version `0.25.0` ships the first Keel-specific adapter as a non-executable planning boundary. Version `0.41.0` adds parameter-free read-only discovery of the supported per-user Linux layout, fixed Docker identity and persistence signals, port 3000 exposure, and exact Keel health identity without returning private paths or reading application secrets. The adapter still recognizes only one exact [Keel Notes](https://github.com/AES256Afro/Keel) Linux x64 release identity, and the plan cannot be staged. See [the Keel adapter guide](docs/KEEL.md) for the discovery boundary, fixed artifact identity, and acceptance gates required before execution can be enabled.
