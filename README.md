...
| Area | Status in `0.47.0` | Capability |
| --- | --- | --- |
...
| Keel Notes adapter | Guarded acquisition, staging, private install, and terminal claim handoff | Inspects bounded existing-install evidence, then locally verifies and safely stages only fixed `v1.2.6`. A separate password-approved plan creates one non-login account, private `/var/lib/keel` state, exact activation link, and hardened systemd unit. It starts only on `127.0.0.1:3000` and requires the exact health identity and private SQLite file. Failure preserves state. A fixed fresh-sudo terminal tool safely claims the chosen Keel account without sending the token through BoxPilot. Registration changes, Tailscale exposure, backup, restore, import, adoption, update, and removal remain unavailable. |
