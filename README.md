...
| **App catalog** | 159 self-hosted apps and game servers: search, install, configure, update, back up, restore, uninstall. What is running sits at the top. Every app with a login has a Sign in panel — the page, the username, the password, and a way to change it. Apps that support it (Pi-hole) can run in Host network mode so they see each device on your LAN, not one shared address. Each app is reachable from your home network or only through Tailscale — one switch, with HTTPS and a real certificate. |
