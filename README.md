...
Version `0.37.0` extends the fixed Flint 2 gateway acceptance to one enrolled LAN device. After a fresh passing `0.36.0` Bigbox record, the owner can schedule only an immediate, 5-minute, or 10-minute one-shot task. The Ed25519 agent re-derives its own local default gateway and rejects the task unless it exactly matches the controller record, then runs the same four fixed TCP and UDP queries. It accepts no address, hostname, command, credential, router setting, recurrence, or arbitrary execution time. Passing evidence proves only the signed device-to-observed-gateway DNS path during that window. Physical model identity, AdGuard configuration, DHCP advertisement, filtering behavior, and router or DNS cutover remain unproven and unavailable.

### What works now

| Area | Status in `0.37.0` | Capability |
| --- | --- | --- |
