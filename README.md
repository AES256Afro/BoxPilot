...
| Disaster recovery kit | Authenticated read-only export | Correlates sanitized job, controller-backup, application-backup, protected-VM-backup, router-checkpoint, migration, fleet, DNS, and prerequisite evidence. A verified local controller snapshot changes the database check from missing to operator-check because independent storage remains unproved. JSON and Markdown downloads remain evidence only: no database, application data, configuration file, backup payload, credential, or mutation is included. |
| Restricted helper | Live typed operations | Uses a versioned, allowlisted protocol over a local Unix socket for the fixed `smartmontools` inspection and exact-version install, fixed APT metadata inspection and refresh, canary, bounded inventory and logs, WAL-aware controller backup, Uptime Kuma deployment and backup, exact-address Pi-hole staging and backup, guarded local migration staging, guarded VM creation and lifecycle, read-only libvirt inventory, offline snapshots, stopped-VM exports, mounted-restic VM copies, isolated restore drills, stopped no-network recovery clones, and exact no-prune retention. The controller operation accepts only a server-generated UUID and derives the source, artifact, manifest, and drill paths. It accepts no general package name, repository, command string, binary selection, libvirt URI, argument array, operator path, Compose source path, migration destination, SSH credential, repository password, backup mount, repository path, export destination, restore destination, recovery directory, prune flag, selector such as `latest`, or arbitrary root path from the browser. |
...
| Backup engine | Controller plus two application adapters | Creates a no-downtime WAL-aware BoxPilot SQLite snapshot and immutable local Uptime Kuma and Pi-hole archives. The controller workflow verifies a separate copy-open drill, while application workflows verify source restart and temporary restore containers with no network or published ports. Pi-hole evidence additionally requires configuration, secret, and no-cutover proof. All local artifacts remain same-host recovery evidence, not independent copies. |
...
- Backup schedules, controller and application-backup independent or offsite destinations, browser download, automatic controller restore, restic prune and space reclamation, configurable retention policies, remote restic/cloud backends, Keel Notes export, SSH source discovery or transport, general application-aware volume/database capture, staged-workload activation, or migration cutover
...
- [WAL-aware BoxPilot controller backup and recovery runbook](docs/CONTROLLER-BACKUPS.md)
