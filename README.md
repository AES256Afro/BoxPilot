...
| **App catalog** | Install, configure, update, back up, restore, and uninstall 128 self-hosted apps from YAML manifests — Immich, PhotoPrism, Nextcloud, Jellyfin, Plex, Emby, Mosquitto/Zigbee2MQTT/Z-Wave JS UI/Node-RED/ESPHome for the smart home, Netdata, Prometheus, Loki, Dozzle, Healthchecks, WireGuard, Nginx Proxy Manager, Cloudflare Tunnel, a Matrix server, Mattermost, wikis (BookStack, Wiki.js, Docmost), Actual Budget, Firefly III, Grocy, Tandoor, databases with admin UIs (PostgreSQL, MariaDB, MongoDB, InfluxDB, Valkey), NocoDB, Baserow, Metabase, Semaphore, game servers (Terraria, Factorio, Satisfactory, Palworld, PufferPanel, RomM), the Sonarr/Radarr/Lidarr/Prowlarr/Bazarr stack with qBittorrent behind a VPN, Home Assistant, Vaultwarden, Forgejo, Portainer, Grafana, Beszel, Paperless-ngx, n8n, Open WebUI + Ollama with a model picker, SearXNG, a Minecraft server, Pi-hole and AdGuard Home (each with a bundled Unbound resolver and, for Pi-hole, a blocklist picker), Scrutiny disk health, Duplicati and Kopia backups, MinIO, Stirling PDF, Kavita, Komga, Calibre-Web, File Browser, and more. Live status, logs, resource use, one-click HTTPS on your tailnet. |
...
| **Storage** | Disks and LVM (grow the root volume with one click), mounting by UUID, SMB/NFS network shares with LAN discovery and self-reconnecting mounts, Samba and NFS servers that share this server's folders over your tailnet only (or the LAN), swap files. The system disk can never be formatted from the UI. |
