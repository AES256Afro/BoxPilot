...
| Area | Status in `0.46.0` | Capability |
| --- | --- | --- |
...
| Keel Notes adapter | Discovery, guarded acquisition, and inert release staging | Inspects bounded native user-service and Docker evidence, fixed port 3000 posture, persistence signals, and exact Keel health identity. Separate empty-input acquisition and safe-staging plans can locally verify and then extract only the fixed `v1.2.6` Linux x64 archive after separate staging and password approval. The runtime and extracted-tree gates require 2,974 logical members, 2,466 regular files, 508 directories, and no links, special members, state, or secrets. The published release tree is root-only and inert. No service, writable state, account, registration, listener, program execution, backup, restore, or exposure is created. |
...
- Keel Notes installation, activation, claim, backup, restore, import, adoption, update, and exposure; executable AdGuard Home, Jellyfin, Home Assistant, PostgreSQL, Pi-hole router cutover, private or write-capable GitHub integration, signed adapter installation, general remote-agent operations, automatic remediation, persistent alerts, or external notification delivery
...
### Keel Notes safe release staging mockup

![BoxPilot guarded Keel Notes 1.2.6 inert release staging plan](docs/screenshots/keel-plan-mock.png)

This explicitly disclosed `0.46.0` mock shows the fixed Keel 1.2.6 identity, public and complete local-byte verification, exact pre-extraction membership, post-extraction root-only checks, password handoff, generated-partial rollback, and the continuing no-install boundary. No request, password, download, archive extraction, application tree, state, account, service, listener, registration, network, or host change occurred for the capture.
