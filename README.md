...
| Area | Status in `0.49.0` | Capability |
| --- | --- | --- |
...
| Keel Notes adapter | Guarded acquisition, staging, private install, terminal claim, backup, and stopped recovery clone | Inspects bounded existing-install evidence, then locally verifies and safely stages only fixed `v1.2.6`. Separate password-approved install and backup plans use dedicated static units. Keel runs as one non-login account on `127.0.0.1:3000`; the backup stops only that unit, runs the fixed upstream export as `keel`, restarts and health-checks the source, and verifies a root-only archive in an isolated SQLite-open drill. A separate high-risk plan can materialize that exact verified archive as a new stopped, no-network, root-only recovery state without replacing production. The terminal claim token never enters BoxPilot. Registration changes, Tailscale exposure, production restore, import, adoption, update, removal, and recovery promotion remain unavailable. |
...
- Keel Notes claim automation, registration verification or change, Tailscale exposure, backup, restore, import, adoption, update, and removal; executable AdGuard Home, Jellyfin, Home Assistant, PostgreSQL, Pi-hole router cutover, private or write-capable GitHub integration, signed adapter installation, general remote-agent operations, automatic remediation, persistent alerts, or external notification delivery
