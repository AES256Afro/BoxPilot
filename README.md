...
```bash
curl -fsSL https://raw.githubusercontent.com/AES256Afro/BoxPilot/main/scripts/boxpilot-install.sh | sudo sh -s -- --ref v1.8.2
```
...
| **App catalog** | 159 self-hosted apps and game servers: search, install, configure, update, back up, restore, uninstall. What is running sits at the top. Every app with a login has a Sign in panel — the page, the username, the password, and a way to change it. Each app is reachable from your home network or only through Tailscale — one switch, with HTTPS and a real certificate. |
