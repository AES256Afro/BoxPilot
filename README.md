...
| Area | Status in `0.51.0` | Capability |
| --- | --- | --- |
...
| Keel Notes adapter | Guarded acquisition, staging, private install, terminal claim, backup, drilled recovery, and rollback-backed promotion | Inspects bounded existing-install evidence, then locally verifies and safely stages only fixed `v1.2.6`. Separate password-approved install and backup plans use dedicated static units. Keel runs as one non-login account on `127.0.0.1:3000`; the backup stops only that unit, runs the fixed upstream export as `keel`, restarts and health-checks the source, and verifies a root-only archive in an isolated SQLite-open drill. Separate high-risk plans materialize a stopped recovery and run a disposable private-namespace startup rehearsal. The critical promotion job then atomically retains the stopped prior production directory, activates only the exact drilled state, and automatically restores the old state on failure. The terminal claim token never enters BoxPilot. Owner-login proof, private exposure, import, adoption, update, removal, and rollback-checkpoint retention remain unavailable. |
...
| Backup engine | Controller plus three application adapters and guarded Keel recovery promotion | Creates a no-downtime WAL-aware BoxPilot SQLite snapshot and immutable local Uptime Kuma, Pi-hole, and Keel archives. Keel adds clean native-service export, guaranteed source restart, a complete manifest and tree digest, an isolated restored-SQLite drill, a stopped recovery clone, a private-namespace startup rehearsal, and rollback-backed atomic production promotion. Controller, application, and VM protection use separate fixed restic repositories and recovery passwords. Application protection requires the earlier adapter-aware drill, then performs a complete repository read and byte-for-byte exact archive restore. Controller retention exists; application and Keel rollback retention do not. |
...
- Keel Notes claim automation, independent registration verification, Tailscale exposure, owner-login proof, import, adoption, update, removal, rollback-checkpoint retention, and operator-requested rollback; executable AdGuard Home, Jellyfin, Home Assistant, PostgreSQL, Pi-hole router cutover, private or write-capable GitHub integration, signed adapter installation, general remote-agent operations, automatic remediation, persistent alerts, or external notification delivery
...
![BoxPilot guarded Keel Notes 1.2.6 private native-service installation plan](docs/screenshots/keel-plan-mock.png)
...
![BoxPilot guarded Keel Notes consistent export and isolated recovery plan](docs/screenshots/keel-backup-approval-mock.png)
...
![BoxPilot guarded Keel Notes stopped recovery-clone plan](docs/screenshots/keel-recovery-clone-mock.png)
...
![BoxPilot guarded Keel Notes isolated startup rehearsal](docs/screenshots/keel-recovery-drill-mock.png)
...
![BoxPilot guarded Keel Notes rollback-backed production promotion](docs/screenshots/keel-promotion-mock.png)
...
- [Keel Notes discovery, exact-release staging, private installation, and claim handoff](docs/KEEL.md)
