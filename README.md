...
| Restricted helper | Live typed operations | Uses a versioned, allowlisted protocol over a local Unix socket for fixed prerequisite, inventory, controller backup, independent protection and retention, curated application, migration, and VM workflows. Controller protection accepts only two UUIDs, two recorded hashes, an exact size, and a destination revision. Controller retention accepts only a server-generated UUID, fixed repository and destination evidence, the complete snapshot-set revision, and one to 100 exact sorted snapshot ids selected by server policy. It derives the mount, repository, password file, source, restore workspace, path, and restic arguments. No browser password, path, repository, selector, command, endpoint, retention rule, forget option, or prune input exists. |
