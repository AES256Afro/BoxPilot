...
| **App catalog** | Install, configure, update, back up, restore, and uninstall 52 self-hosted apps from YAML manifests — Immich, PhotoPrism, Nextcloud, Jellyfin, Plex, Emby, the Sonarr/Radarr/Lidarr/Prowlarr/Bazarr stack with qBittorrent behind a VPN, Home Assistant, Vaultwarden, Forgejo, Portainer, Grafana, Beszel, Paperless-ngx, n8n, Open WebUI + Ollama with a model picker, SearXNG, a Minecraft server, Pi-hole and AdGuard Home (each with a bundled Unbound resolver and, for Pi-hole, a blocklist picker), Scrutiny disk health, Duplicati and Kopia backups, MinIO, Stirling PDF, Kavita, Komga, Calibre-Web, File Browser, and more. Live status, logs, resource use, one-click HTTPS on your tailnet. |
