...
Version `0.35.0` adds a durable, password-approved, metadata-only APT refresh. Repair Center offers it only when metadata is stale or unavailable and dpkg state is ready. A static no-argument root unit runs exactly `apt-get update --error-on=any`, then proves the installed package database is unchanged and metadata is current. No package, repository, command, option, or target comes from the browser. Package install, upgrade, removal, service control, update-policy changes, and reboot remain unavailable except for the older exact-version `smartmontools` repair.

### What works now

| Area | Status in `0.35.0` | Capability |
| --- | --- | --- |
