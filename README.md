...
Version `0.39.0` adds encrypted independent protection for a verified local BoxPilot controller snapshot. A second immutable plan accepts only the selected backup id, server-generated protection id, recorded hashes and size, and a fixed destination revision. The restricted helper derives every path, requires a writable exact mount at `/mnt/boxpilot-backup` on a filesystem different from both controller-state locations, uses a separate root-only recovery password and `restic-controller` repository, reverifies the local artifact and manifest, reads every repository data pack, confirms the exact snapshot identity, restores it into a no-network helper-owned workspace, repeats both hashes and all SQLite checks, and removes the successful drill workspace. Only that complete evidence promotes the snapshot from locally verified to independently protected. Bigbox currently has no independent mount configured, so the live UI fails closed with terminal setup guidance and makes no protection claim.

### What works now

| Area | Status in `0.39.0` | Capability |
| --- | --- | --- |
...
| Restricted helper | Live typed operations | Uses a versioned, allowlisted protocol over a local Unix socket for fixed prerequisite, inventory, controller backup and independent protection, curated application, migration, and VM workflows. Controller protection accepts only two UUIDs, two recorded hashes, an exact size, and a destination revision. It derives the mount, repository, password file, source, restore workspace, path, and restic arguments. No browser password, path, repository, selector, command, endpoint, retention rule, forget, or prune input exists. |
...
- Backup schedules, independent application-backup destinations, controller remote/cloud adapters, browser download, automatic controller production restore, restic prune and space reclamation, configurable controller retention, remote restic/cloud backends, Keel Notes export, SSH source discovery or transport, general application-aware volume/database capture, staged-workload activation, or migration cutover
