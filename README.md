...
| **App catalog** | Install, configure, update, back up, restore, and uninstall 75 self-hosted apps from YAML manifests — Immich, PhotoPrism, Nextcloud, Jellyfin, Plex, Emby, Mosquitto/Zigbee2MQTT/Z-Wave JS UI/Node-RED/ESPHome for the smart home, Netdata, Prometheus, Loki, Dozzle, Healthchecks, WireGuard, Nginx Proxy Manager, Cloudflare Tunnel, a Matrix server, the Sonarr/Radarr/Lidarr/Prowlarr/Bazarr stack with qBittorrent behind a VPN, Home Assistant, Vaultwarden, Forgejo, Portainer, Grafana, Beszel, Paperless-ngx, n8n, Open WebUI + Ollama with a model picker, SearXNG, a Minecraft server, Pi-hole and AdGuard Home (each with a bundled Unbound resolver and, for Pi-hole, a blocklist picker), Scrutiny disk health, Duplicati and Kopia backups, MinIO, Stirling PDF, Kavita, Komga, Calibre-Web, File Browser, and more. Live status, logs, resource use, one-click HTTPS on your tailnet. |
