...
| **Storage** | Disks and LVM (grow the root volume with one click), mounting by UUID, SMB/NFS network shares with LAN discovery and self-reconnecting mounts, a Samba file server that shares this server's folders over your tailnet only (or the LAN), swap files. The system disk can never be formatted from the UI. |
