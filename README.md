...
| Area | Status in `0.42.0` | Capability |
| --- | --- | --- |
...
| Keel Notes adapter | Discovery plus guarded inert artifact acquisition | Inspects bounded native user-service and Docker evidence, fixed port 3000 exposure, persistent-data signals, and exact Keel health identity. A separate empty-input plan can acquire and locally verify only the fixed `v1.2.5` Linux x64 archive after staging and password approval. It exposes no username, private path, redirect credential, archive byte, container id, environment, database content, or secret. Extraction, installation, adoption, start, claim, backup, restore, import, and exposure remain locked. |
...
This explicitly disclosed `0.42.0` mock shows a clean absent-install discovery result, the fixed Keel release identity, GitHub metadata match, an immutable empty-input artifact plan, the separate static downloader boundary, owner-password handoff, and the extraction and installation lock. It illustrates the product state only: no request, password, marker, archive, local hash, evidence file, application tree, account, service, container, port, or data change occurred for the capture.
