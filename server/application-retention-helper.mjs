...
export function validateApplicationRetentionInput(input) {
  const errors = [];
  if (!input || typeof input !== "object" || Array.isArray(input)) return ["An application retention request is required"];
  ...
  if (keys.length !== allowedKeys.length || keys.some((key, index) => key !== allowedKeys[index])) errors.push("Application retention accepts only fixed typed evidence fields");
...
export function createApplicationRetentionHelper({
  ...
  inspectDestination = createApplicationProtectionHelper().inspect,
  run = defaultRunner,
} = {}) {
  ...
  if ((!resolvedMountRoot.startsWith("/mnt/") && !resolvedMountRoot.startsWith("/media/")) || resolvedMountRoot === "/mnt" || resolvedMountRoot === "/media") {
    throw new Error("The application backup mount must be a dedicated path below /mnt or /media");
  }
  const repository = path.join(resolvedMountRoot, "restic-applications");
  if (path.dirname(repository) !== resolvedMountRoot || path.basename(repository) !== "restic-applications") throw new Error("The application restic repository escaped the configured mount");
  ...
  async function inspect() {
    ...
    if (!destination.ready || !shaPattern.test(destination.repositoryId ?? "") || !shaPattern.test(destination.destinationRevision ?? "")) {
      return {
        ...
        snapshots: [],
        blockers: destination.blockers ?? ["The encrypted application backup destination is unavailable"],
      };
    }
    const result = await run(resticBinary, [...commonResticArguments(), "snapshots", "--json", "--tag", "boxpilot-application"], { timeout: 30000 });
    const parsed = JSON.parse(result.stdout || "[]");
    if (!Array.isArray(parsed) || parsed.length > 10000) throw new Error("Restic returned an invalid application snapshot inventory");
    const snapshots = parsed.map((snapshot) => {
      if (!shaPattern.test(snapshot.id ?? "") || typeof snapshot.time !== "string" || !Number.isFinite(Date.parse(snapshot.time)) || !Array.isArray(snapshot.tags)
        || snapshot.tags.some((tag) => typeof tag !== "string" || tag.length > 128) || !snapshot.tags.includes("boxpilot-application")) {
        throw new Error("Restic returned invalid application snapshot evidence");
      }
      return { id: snapshot.id, time: snapshot.time, tags: [...snapshot.tags].sort() };
    }).sort((left, right) => left.id.localeCompare(right.id));
    if (new Set(snapshots.map((snapshot) => snapshot.id)).size !== snapshots.length) throw new Error("Restic returned duplicate application snapshot ids");
    return {
      ready: true,
  ...
  async function apply(parameters) {
    const errors = validateApplicationRetentionInput(parameters);
    ...
    if (!before.ready || before.repositoryId !== parameters.repositoryId
      ...
      throw new Error("The encrypted application repository or its snapshot inventory changed after approval");
    }
    const beforeIds = new Set(before.snapshots.map((snapshot) => snapshot.id));
    if (parameters.forgetSnapshotIds.some((id) => !beforeIds.has(id))) throw new Error("A reviewed application snapshot is no longer present in the repository");
    ...
    if (actuallyForgotten.length === 0) throw new Error("Application retention failed before any reviewed snapshot removal was confirmed");
...
export const applicationRetentionHelperInternals = { defaultRunner, snapshotSetRevision };
