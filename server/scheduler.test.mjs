...
describe("operation scheduler", () => {
  ...
  it("refuses high-risk, read-only, unknown, and invalid-parameter schedules", async () => {
    ...
    expect(() => scheduler.create({ ...base, operationId: "apt.purge" })).toThrow("high risk");
    expect(() => scheduler.create({ ...base, operationId: "app.inspect" })).toThrow("Read-only");
    expect(() => scheduler.create({ ...base, operationId: "nope" })).toThrow("not registered");
    expect(() => scheduler.create({ ...base, operationId: "app.backup", parameters: {} })).toThrow("requires id");
    expect(() => scheduler.create({ ...base, operationId: "apt.refresh", minute: 99 })).toThrow("minute");
  });
  ...
  it("runs due schedules as their creator and advances the next occurrence", async () => {
    ...
    const schedule = scheduler.create({ operationId: "app.backup", parameters: { id: "jellyfin" }, frequency: "daily", minute: 0, hour: 3, createdBy: owner.id });
  ...
  it("records a skipped run instead of forcing one when approvals demand a password", async () => {
    ...
    const schedule = scheduler.create({ operationId: "apt.refresh", parameters: {}, frequency: "hourly", minute: 0, createdBy: owner.id });
