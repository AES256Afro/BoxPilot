...
describe("cloud backup operations", () => {
  it("validates destinations per provider, marks credentials secret, and stages the tasks", async () => {
    ...
    expect(validateParameters(setup, { provider: "b2", account: "0012abc", bucket: "home-backups", path: "bigbox", key: "K" }, "t")).toBeNull();
    ...
    await operations["backup.cloud.setup"].run({ provider: "b2", account: "a", bucket: "home-backups", path: "bigbox", key: "K" }, { runUnit, jobLog: null });
    expect(runUnit.runTask).toHaveBeenCalledWith("backup.cloud.setup", expect.objectContaining({ provider: "b2", key: "K" }), expect.anything());
    await operations["backup.cloud.sync"].run({ provider: "b2", account: "a", bucket: "home-backups", path: "bigbox", key: null }, { runUnit, jobLog: null });
    expect(runUnit.runTask).toHaveBeenLastCalledWith("backup.cloud.sync", { provider: "b2", account: "a", bucket: "home-backups", path: "bigbox" }, expect.anything());
