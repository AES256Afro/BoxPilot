...
const readOnlyOperations = new Set(["canary.verify", "prerequisite.smartmontools.inspect", "prerequisite.apt-metadata.inspect", "container.docker.inspect", "container.docker.inventory", "system.logs.inspect", "controller.database.backup.inspect", "controller.database.protection.inspect", "application.uptime-kuma.inspect", "application.pi-hole.inspect", "virtualization.inventory.inspect", "virtualization.console.inspect", "virtualization.domain.export.inspect", "virtualization.export.backup.inspect", "virtualization.export.backup.retention.inspect", "virtualization.export.backup.restore-drill.inspect", "virtualization.backup.recovery.inspect"]);
...
const helperDependencies = { applications, controllerBackups, controllerProtection, migrations, prerequisites, vmRestoreDrill, vmRecovery, vmRetention };
...
server.listen(socketPath, async () => {
  ...
  console.log(`BoxPilot helper 0.39.0 listening on ${socketPath}`);
});
