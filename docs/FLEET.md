...
The agent first authenticates a poll. It rejects any task that changes the schema, resolver source, four fixed names, protocols, port 53, expected response codes, or the explicit no-command and no-cutover boundary. It then performs:
