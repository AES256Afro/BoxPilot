...
BoxPilot `0.42.0` provides integrity-addressed manifests, two executable deployment adapters, two application-aware backup adapters, and a Keel Notes adapter with read-only discovery plus a separately approved inert artifact acquisition. Keel combines bounded host evidence, exact public release metadata, and complete local archive hashing but cannot extract, execute, install, adopt, start, claim, back up, restore, import, or expose Keel. The web process never receives the Docker socket. Docker readiness, executable application inspection, deployment, backup, restore-drill execution, and parameter-free Keel discovery and artifact inspection cross the restricted local helper as typed operations. The main helper remains network-isolated and delegates the fixed Keel download to a static sandboxed one-shot.
...
The Keel Notes deployment entry remains deliberately non-executable. It accepts only the declared `native-service` target and loopback web port, rejects undeclared fields, checks the Linux x64 host identity and prerequisite state, and compares the fixed release tag, commit, asset name, byte count, and GitHub-reported SHA-256 value with the live fixed public GitHub metadata view. Every deployment plan includes `keel.execution` as an unresolved blocker, so no deployment stage or approval route can be created. Existing or ambiguous installation evidence adds separate blockers for adoption or migration review. Artifact acquisition uses its own empty-input plan, stage route, and durable job.

GitHub metadata matching is not local verification. Only a completed artifact job reports local verification after hashing all fixed bytes. The exact identity and the full set of remaining extraction, service, claim, and recovery gates are documented in [Keel Notes planning](KEEL.md).
