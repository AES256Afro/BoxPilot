...
- ✅ (partial) **M1.1** `server/ops/registry.mjs` (declarative param spec; JSON Schema can replace it later). Canary + 10 prerequisite ops ported; allowlist, read-only set, and timeouts derive from registry ∪ legacy lists with a no-overlap test. `GET /api/v1/operations`. The prerequisite family (canary + 5 repairs) and the VM lifecycle family (`virtualization.domain.action` → registry op `vm.action`: start, graceful two-minute shutdown, reboot, autostart, with in-op state guards; `vm-lifecycle.mjs` deleted, `jobs.mjs` at 854 lines) are fully ported end to end: Repair Center stages `op:` jobs through the shared dialog, the plan/stage routes and `prerequisite-repairs.mjs` are deleted, and `jobs.mjs` lost their allowlist entries and execution literals (995 → 865 lines). Remaining: ~32 legacy job types (applications, keel, backups, remaining VM workflows, migration, network), then delete the legacy lists.
