...
- ✅ (mostly) **M1.4** Generic job path is the only path: `POST /api/v1/operations/:id/jobs` stages any registered mutating op as `op:<id>`; approval/execution are generic (`jobs.createOperationJob`), `GET /api/v1/operations/:id/inspect` runs read-only ops directly; the legacy executor branch is deleted. Remaining: split `index.mjs` into routers.
