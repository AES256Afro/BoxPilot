...
- ✅ (v1) **M2.3** Automatic-updates toggle on the Updates page: `apt.unattended.inspect` + `apt.unattended.set` (installs unattended-upgrades when needed, writes `20auto-upgrades`). Remaining: schedule customization, `needrestart` integration.
...
- ◐ **M2.9** Docker housekeeping on the System page: `docker.disk.inspect` (system df) + `docker.prune` (system prune, never volumes). Portainer and Dockge are already catalog items. Remaining: Engine from the official repo, compose plugin, log rotation defaults.
...
- ◐ **M3.3** Generated config forms exist (M3.2); a **Config** button on each installed app now shows the effective `.env` (secrets masked) and `compose.yaml` via `app.config.inspect`. Remaining: "Edit raw" for power users (medium risk).
