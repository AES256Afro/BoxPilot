...
- ✅ (partial) **M1.1** `server/ops/registry.mjs` (declarative param spec; JSON Schema can replace it later). Canary + 10 prerequisite ops ported; allowlist, read-only set, and timeouts derive from registry ∪ legacy lists with a no-overlap test. `GET /api/v1/operations`. The prerequisite family (canary + 5 repairs) the VM lifecycle family (`virtualization.domain.action` → registry op `vm.action`), and offline snapshots (`virtualization.domain.snapshot.create` → `vm.snapshot.create` with in-op qcow2/backing-chain/stopped guards) are fully ported end to end; `vm-lifecycle.mjs` deleted, `vm-snapshot.mjs` trimmed to its revision helpers, `jobs.mjs` at 843 lines: Repair Center stages `op:` jobs through the shared dialog, the plan/stage routes and `prerequisite-repairs.mjs` are deleted, and `jobs.mjs` lost their allowlist entries and execution literals (995 → 865 lines). After the big excision and the foundation/controller ports, 7 legacy job types remain — the VM media/create/export/protection/retention/restore-drill/recovery workflows — then the legacy lists go. New generic mechanisms: `operationPrepareHooks` (server pins recorded-evidence expectations into staged parameters at staging time) and `operationRecordHooks` (results become durable evidence rows).
...
- ✅ (v1) **M2.6** **Firewall** page (`src/FirewallCenter.tsx`): ufw state and rules read from its config files (the helper's PrivateNetwork hides live iptables), enable/disable (high; enabling always adds SSH 22/tcp + `allow in on tailscale0` first), add/delete port rules (medium; the SSH rule is undeletable), install-ufw path via `apt.install`. Ops `firewall.*` → root tasks in `server/tasks/firewall.mjs`. Remaining: profile presets, per-app rules wired into app install/uninstall.
