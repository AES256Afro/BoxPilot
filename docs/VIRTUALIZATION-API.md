...
The BoxPilot `v1` virtualization API is loopback-only by default. Tailscale Serve may proxy it privately, but it is not an internet API. Version `0.22.0` requires an authenticated owner session for virtualization routes and a CSRF token for POST requests. Network privacy remains mandatory. Version `0.22.0` does not change the virtualization route contract.
