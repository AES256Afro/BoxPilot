...
Version `0.21.0` adds a non-mutating Pi-hole acceptance boundary after guarded staging and restore proof. The browser supplies no DNS target or query. The server derives the exact managed resolver, links the original assessment, deployment, and backup, revalidates live topology, and requires a separate password-approved durable job. The unprivileged web process sends four fixed direct DNS queries and records structured evidence. The privileged helper remains network-isolated. No router, DHCP, client DNS, Tailscale, firewall, or traffic-path mutation handler exists.
...
The persistent store is SQLite. Owners, sessions, jobs, job steps, approvals, plans, application backups, DNS acceptance runs, VM exports, VM backups, VM recoveries, VM retention runs, imported migration sources, verified migration transfers, and audit events are live. Planned records include:
