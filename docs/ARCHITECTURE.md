...
Version `0.51.0` adds the critical Keel production-promotion boundary. The web process can select only a durable stopped recovery whose latest matching drill passed. The plan pins a generated promotion id, recovery id, drill id, evidence checksum, full state digest, and exact healthy install id. The helper delegates that tuple to one static no-argument unit. The unit creates a generated candidate, stops only Keel, checks the old and new databases, requires one filesystem, and uses atomic whole-directory moves to retain the entire stopped old state before activating the drilled state. Exact health and repeated SQLite checks are required. A root-only persistent phase marker lets the same unit restore old production after an interrupted exchange. The unit accepts no path, command, token, port, service, network, or rollback destination. Its `ReadWritePaths=/var/lib` allowance is intentionally broad enough for atomic directory exchange, so the no-argument script, exact paths, membership checks, persistent journal, and automatic rollback are security-critical controls.

Version `0.52.0` adds a separate operator-requested rollback boundary. The web process selects only a durable completed promotion. The immutable plan pins a generated rollback id plus the promotion, current installation, original checkpoint digest, and evidence checksum. The static no-argument unit copies the original checkpoint, atomically displaces current production, activates the copy, and requires exact health and SQLite proof. The original promotion checkpoint is read-only to the unit and never moved. A second root-only checkpoint retains displaced current production. Persistent phases let the same unit restore displaced production after a failed or interrupted exchange. No path, command, token, service, port, network, or retention choice crosses the helper protocol.

Version `0.53.0` adds a terminal-only Keel instance-owner login boundary outside the web process and helper mutation protocol. A root parent validates only the fixed managed install, then starts a worker that drops every supplemental group and changes permanently to the dedicated `keel` gid and uid before reading credentials from the terminal. Only that worker sees the email, hidden password, temporary cookie jar, owner-route response, and logout form. It accepts no command-line, environment, browser, path, endpoint, or network input. The parent receives one exact sanitized structure over a local IPC channel and can atomically write only a root-owned mode-0600 fixed proof file. Parameter-free helper inspection never reads credentials, cookies, application state, the Keel database, or a secret. WebAuthn redirects fail closed without proof.

Version `0.54.0` adds Docker Engine as a first-class prerequisite repair rather than a setup-plan placeholder. Read-only inspection is a distinct parameter-free helper operation and treats an active existing compatible provider as complete. A present but inactive or unrecognized provider blocks automatic installation for manual repair. Only a host with no fixed Docker client path or installed `docker.io` package and one configured Ubuntu candidate can produce an immutable plan. Approval revalidates that no provider appeared and that the exact candidate is unchanged. The helper then delegates to a static no-argument unit through a five-minute root-only marker. That unit alone can install the pinned package, enable and start `docker.service`, and read the local daemon version. The browser cannot select a package, repository, daemon configuration, user, socket, image, container, or command, and the workflow has no removal or provider-replacement path.

Version `0.55.0` adds a separate virtualization prerequisite boundary for a clean Ubuntu host. Parameter-free inspection reads only fixed dpkg and APT evidence, the two fixed provider paths, fixed KVM kernel evidence, `libvirtd.service`, QEMU version output, and `qemu:///system`. An active exact package set is ready; any partial provider blocks automatic installation. Only hardware-ready provider-free state with candidates for the five fixed roots can produce a plan. The plan pins every root version and discloses that Ubuntu may change required dependencies. A static no-argument unit requires the actual host `/dev/kvm`, rechecks the complete state, installs only the pinned roots, enables and starts only `libvirtd.service`, and verifies hardware, QEMU, service, and URI. It cannot edit an operator user or group, create a network, pool, disk, ISO attachment, or VM, replace a partial provider, or accept a browser command, URI, path, resource name, or package selection.

Version `0.55.1` keeps the main helper's `PrivateDevices=true` boundary intact and reads KVM registration through `/sys/class/misc/kvm/dev` instead of the hidden device node. The fixed path reveals only the registered major and minor device identity. The separately named installation unit remains outside the private-device namespace and still requires `/dev/kvm` at both systemd and script boundaries before APT can run.
...
BoxPilot web and API process (unprivileged)
          ...
          +---- Durable fixed-query Pi-hole DNS acceptance from the server (0.21.0)
...
Version `0.59.0` adds a distinct private-access boundary. Parameter-free inspection first requires the exact managed Uptime Kuma identity, then derives its loopback port and the connected node's tailnet DNS name. It parses bounded Serve JSON plus the CLI's tailnet-only label, rejects an unmanaged listener conflict, and hashes the application revision with every Serve route outside the derived app port. The mutation accepts only Publish or Unpublish plus that complete revision. It invokes one fixed `tailscale serve` argument array, never Funnel, and verifies the expected URL, tailnet-only state, and unchanged non-application configuration. Hostname, port, target, path, protocol, Service name, command, argument, public exposure, firewall, DNS, router, container, and application changes have no input or execution route.

Version `0.60.0` adds a distinct application-retention boundary. Parameter-free inspection is confined to the fixed `restic-applications` repository and `boxpilot-application` tag. The controller correlates every returned snapshot with one active durable protection record and its exact application, backup, and protection tags. Selection keeps three copies per application, a 30-day age floor, incomplete restore evidence, Keel recovery references, and active application jobs. The mutation accepts only the generated retention id, repository and destination revisions, exact snapshot-set revision, and up to 100 sorted snapshot ids. It runs `restic forget` and a complete data read, never prune, and writes durable forgotten-member evidence before a later verification failure can obscure confirmed removal.

Version `0.61.0` adds a split VM-media boundary. The authenticated, CSRF-protected web service may stream one safe `.iso` up to 16 GiB only into `/var/lib/boxpilot-managed/vm-media-inbox`. It derives the target from a basename-only header, enforces declared and received byte counts plus a fixed free-space reserve, hashes the complete stream, and publishes a mode-0600 file plus bounded metadata without overwriting. That staging write cannot publish libvirt media. Parameter-free helper inspection lists only complete regular staging pairs. A separate immutable, staged, password-approved job accepts only a generated import UUID, exact filename, byte count, SHA-256, and candidate revision. The helper derives both roots, rehashes the source, copies into a generated partial beneath `/var/lib/libvirt/boot`, rehashes and publishes without overwrite, verifies the final regular file, and only then removes staging. The web service cannot write the managed library. Import cannot create a VM or change libvirt, networking, firewall, Tailscale, DNS, or router state.

## Version 0.61.0 limitations

- The current Overview is authenticated live inventory. Repository screenshots are explicitly disclosed mock product state only; live server-side captures are not published. Settings remains guidance rather than an editable network configuration surface.
...
- The web process has no direct libvirt or KVM group access. Read-only libvirt inventory, canonical default-foundation initialization, and all shipped VM mutations use typed restricted-helper operations. The foundation mutation delegates to a separate static no-argument unit.
...
- Managed media supports authenticated browser upload into one fixed staging directory and separate approved SHA-256-verified import into the fixed library. It does not download from a URL, select an arbitrary path or destination, automatically acquire vendor images, replace existing media, delete managed media, or verify publisher signatures.
...
- The Keel Notes adapter discovers bounded supported per-user service and exact Docker signals, acquires only fixed 1.2.6 bytes, validates archive membership without extraction, stages the exact root-owned tree, installs that tree as a hardened loopback-only native service, creates a consistent local export with isolated SQLite-open verification, materializes a separately approved stopped root-only recovery clone, starts only a generated disposable copy for rehearsal, can atomically promote only that exact drilled state, and can explicitly restore the retained pre-promotion checkpoint while preserving displaced current production. Inspection does not read `.env`, open the database, read the managed-secret key, or follow user-supplied paths. Web and helper operations do not accept a claim token or login credential. Separate terminal handoffs claim the instance and prove password-based instance-owner login plus logout without retaining the credential or session. WebAuthn terminal proof, PostgreSQL or Litestream detection, Node installation, adoption, independent registration verification, Tailscale exposure, rollback retention, import, migration activation, update, removal, and retained-state deletion remain unavailable.
...
- The Action Center is a transient read-only projection of recovery evidence. It has fixed guidance and view navigation only. It cannot dismiss or persist notices, repair a condition, run a command, install a package, schedule work, request browser notifications, or deliver messages externally. The separately named `smartmontools`, `restic`, Ubuntu `docker.io`, fixed virtualization bundle, and APT metadata workflows exist only in Repair Center.
- Only the exact `smartmontools`, `restic`, Ubuntu `docker.io`, and fixed five-root virtualization repairs, the canonical default libvirt foundation, fixed local controller backup, fixed mounted-restic controller and application protection, exact no-prune controller and per-application retention batches, fixed Uptime Kuma deployment, Start, Stop, Restart, backup, and tailnet-only Serve publication or removal, exact-address Pi-hole staging and backup, fixed Keel artifact, stage, install, backup, stopped recovery-clone, promotion, and operator-rollback jobs, guarded local migration staging, fixed VM media import, fixed Linux VM creation, lifecycle actions, offline internal snapshots, stopped-VM exports, mounted-restic VM copies, exact-snapshot isolated restore drills, guarded recovery clones, and exact no-prune VM retention batches can execute mutations. The Docker repair installs only on a provider-free host and does not configure the daemon, users, images, or containers. The virtualization repair likewise refuses partial providers and does not create networks, pools, disks, media attachments, or VMs. The separate foundation can only define, start, and enable the canonical compatible `default` network and pool with job-limited rollback. The restic repair installs and verifies only the package; it cannot configure storage, keys, or repositories. Network assessments and router checkpoints cannot execute. Pi-hole and Flint 2 direct DNS acceptance are approved fixed read-only jobs in the unprivileged web process and never cross the root helper. Signed agents can repeat only the four fixed Pi-hole checks or four fixed Flint 2 checks, with a mandatory node-local default-gateway match for Flint 2. Uptime Kuma removal and update, backup scheduling, local-archive and rollback-checkpoint retention, configurable retention, prune, remote/cloud adapters, arbitrary URL download, general proxy registration, and general automatic production restore remain unavailable.
...
- Controller, Uptime Kuma, Pi-hole, and Keel backups begin as root-only local artifacts on the server. A controller record is protected only after its separate encrypted repository copy, complete read, and exact database restore drill pass. An application record is protected only after its adapter-aware no-network container or SQLite-open drill plus the separate encrypted repository copy, complete read, and exact restored archive hash pass. Fixed controller and per-application retention can later mark exact old repository references as forgotten while retaining every local artifact; application retention also preserves published Keel recovery references. These workflows require an operator-provided independent mounted filesystem. No such destination is currently configured on the server, so no live controller or application protection or retention mutation exists there.
