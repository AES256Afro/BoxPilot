...
- The Keel Notes adapter discovers bounded supported per-user service and exact Docker signals, acquires only fixed 1.2.6 bytes, validates archive membership without extraction, stages the exact root-owned tree, installs that tree as a hardened loopback-only native service, creates a consistent local export with isolated SQLite-open verification, materializes a separately approved stopped root-only recovery clone, starts only a generated disposable copy for rehearsal, and can atomically promote only that exact drilled state. Promotion stops only Keel, retains the full prior state locally, restores it automatically on failure, and leaves the source recovery unchanged. Inspection does not read `.env`, open the database, read the managed-secret key, or follow user-supplied paths. Web and helper operations do not accept a claim token. The separate interactive terminal handoff forces fresh sudo, rechecks the exact fixed installation, permanently drops to the `keel` account, and then runs the upstream one-use claim transaction. PostgreSQL or Litestream detection, Node installation, adoption, independent registration verification, owner-login proof, Tailscale exposure, operator-requested rollback, rollback retention, import, migration activation, update, removal, and retained-state deletion remain unavailable.
