...
Local validation includes locked, unknown, healthy, pending, malformed and failed-verification cases; exact command arguments; permission checks in the registry; and manual-only UI behavior. A live Linux read-only check passed with healthy package state. Repair execution on an interrupted disposable Ubuntu installation remains a release acceptance step.
