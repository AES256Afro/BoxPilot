...
Status: libvirt VM, network, pool, and Tailscale access discovery is included in `0.2.0`. Version `0.7.0` adds host identity and resources, root storage, LAN addresses, selected systemd services, sanitized Docker resource inventory, and fixed redacted journal sources. Version `0.18.0` adds fixed default-route, systemd-resolved, scoped port 53 listener, and Tailscale resolver observations plus no-change router and DNS assessments. Version `0.21.0` adds exact post-staging Pi-hole listener and recovery-baseline validation plus fixed controller-path DNS evidence. Version `0.22.0` adds separately signed second-device evidence for the same fixed Pi-hole checks. Version `0.30.0` adds sanitized real-mount and block topology, a separate fixed timer-only SMART evidence writer, an authenticated fixed-source support bundle with bounded site-specific redaction, and Ubuntu 22.04, 24.04, and 26.04 fixtures. Version `0.30.1` moves mount collection into that fixed timer and reads the host PID 1 mount table, preventing systemd sandbox mounts from being presented as host state. Version `0.31.0` adds an exact-version durable approved repair that installs only the configured `smartmontools` candidate and verifies a fresh fixed scan. Version `0.32.0` adds mounted ext4 kernel `errors_count` evidence with critical, unavailable, and unsupported fail-closed states. Non-ext4 counters, UPS state, and broader hardware collectors remain pending.
