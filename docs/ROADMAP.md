...
Status: integrity-addressed manifests, live prerequisite and address-scoped TCP and UDP port planning, immutable revisions, executable Uptime Kuma and Pi-hole, guarded Pi-hole recovery, and fixed DNS acceptance are included. Versions `0.41.0` through `0.43.0` add Keel discovery, guarded local acquisition, and runtime archive inspection. Version `0.46.0` pins the corrected Keel 1.2.6 archive and permits inert root-owned staging after all evidence gates pass. Version `0.47.0` installs only that staged release as a dedicated loopback-only native service through a separate immutable, staged, password-approved job. Versions `0.48.0` through `0.52.0` add application-aware backup, stopped recovery, isolated startup rehearsal, exact rollback-backed production promotion, and operator-requested rollback with displaced-state preservation. Version `0.53.0` adds terminal-only password owner-login, owner-route, forced-logout, and revoked-session proof with sanitized root-only evidence. Version `0.57.0` adds exact-identity, revision-bound, password-approved Start, Stop, and Restart actions for the managed Uptime Kuma canary. Version `0.58.0` adds the matching network-critical lifecycle for Pi-hole with independent-resolver recovery guidance, exact DNS and web bindings, fixed configuration and secret preservation, and no router, DHCP, client-DNS, firewall, or Tailscale mutation. Version `0.59.0` adds one fixed tailnet-only Tailscale Serve publication and removal workflow for managed Uptime Kuma, with a derived hostname and port, Funnel off, and every other Serve route revision-locked. Version `0.60.0` adds fixed per-application no-prune retention with three-copy and 30-day floors, recovery-reference preservation, exact tagged-snapshot attribution, and complete repository reads. Uptime Kuma removal and updates, the Pi-hole VM target, Keel claim automation and adoption, router advertisement, full Compose parsing, additional adapters, general proxy registration, import, rollback-checkpoint retention, and update previews remain pending.
