...
### Signed second-device acceptance mockup

![BoxPilot signed one-shot edge-router gateway proof](screenshots/edge-router-second-device-mock.png)
...
This explicitly disclosed `0.27.0` mock shows the recommended edge-router, access-point, and standby topology, a live-shaped gateway-address correlation, checkpoint coverage, operator checks, role-specific handholding, and the credential, discovery, probe, upload, DHCP, DNS, Tailscale, and router-write locks. No router was contacted, identified, logged in to, probed, uploaded from, or changed for the capture.

### Edge-router direct DNS acceptance mockup

![BoxPilot immutable edge-router direct gateway DNS acceptance](screenshots/edge-router-dns-acceptance-mock.png)
