  ...
  return (
    <>
      ...
      <section className="panel storage-evidence-panel">
        <header className="panel-header"><div><strong>Drives</strong><span>Space, health, and errors on every mounted drive</span></div><span className={`status-pill ${storageStatus === "healthy" ? "status-good" : storageStatus === "critical" || storageStatus === "warning" ? "status-warning" : "status-neutral"}`}>{storageStatus}</span></header>
        <div className="storage-evidence-summary">
          ...
          <div><span className="eyebrow">Physical drives</span><strong>{blockDevices.available ? `${physicalDisks.length} detected` : "Could not list drives"}</strong><small>Serials and UUIDs excluded</small></div>
