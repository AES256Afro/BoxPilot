...
export default function HostOverview() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Could not read this server's state");
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        : ups.state === "forced-shutdown" ? "Local UPS reports forced shutdown"
          : ups.reason === "nut-client-not-installed" ? "NUT client is not installed"
            : ups.reason === "no-local-ups-configured" ? "No local UPS is configured"
              : ups.configured ? "Cannot read the UPS" : "No UPS is set up";
  ...
  return (
    <>
      ...
      <section className="panel storage-evidence-panel">
        <header className="panel-header"><div><strong>Disks and filesystems</strong><span>Space, health, and errors on every mounted drive</span></div><span className={`status-pill ${storageStatus === "healthy" ? "status-good" : storageStatus === "critical" || storageStatus === "warning" ? "status-warning" : "status-neutral"}`}>{storageStatus}</span></header>
        <div className="storage-evidence-summary">
          <div><span className="eyebrow">Filesystems</span><strong>{filesystems.available ? `${filesystems.mounts.length} real mounts` : "Could not read mounts"}</strong><small>{filesystems.errors.critical} errors recorded | {filesystems.errors.unavailable} unavailable | {filesystems.errors.unsupported} unsupported</small></div>
          <div><span className="eyebrow">Physical disks</span><strong>{blockDevices.available ? `${physicalDisks.length} detected` : "Could not list drives"}</strong><small>Serials and UUIDs excluded</small></div>
          <div><span className="eyebrow">Disk health</span><strong>{smart.available ? `${smart.disks.length} disk results` : smart.reason.replaceAll("-", " ")}</strong><small>{smart.generatedAt ? `${smart.stale ? "Stale" : "Collected"} ${new Date(smart.generatedAt).toLocaleString()}` : "Install smartmontools to read disk health"}</small></div>
        </div>
        {filesystems.mounts.length > 0 && <div className="mount-grid">{filesystems.mounts.map((mount) => <article key={`${mount.target}-${mount.source}`}><div><strong>{mount.target}</strong><span className={`status-pill status-${mount.capacityState === "healthy" ? "good" : mount.capacityState === "unavailable" ? "neutral" : "warning"}`}>{mount.usedPercent === null ? "unknown" : `${mount.usedPercent}% used`}</span></div><p>{mount.source} | {mount.filesystem} | {mount.totalBytes === null ? "size unavailable" : gib(mount.totalBytes)}</p><small>{mount.readOnly ? "read-only" : "read-write"} | {mount.optionNames.length ? mount.optionNames.join(", ") : "no mount options reported"}</small><small className={`filesystem-error filesystem-error-${mount.errorEvidence.state}`}>{mount.errorEvidence.state === "healthy" ? `ext4 kernel errors: ${mount.errorEvidence.errorsCount}` : mount.errorEvidence.state === "critical" ? `ext4 kernel errors recorded: ${mount.errorEvidence.errorsCount}` : mount.errorEvidence.state === "unsupported" ? `${mount.filesystem} error counter unsupported` : "Error check not available for this drive"}</small></article>)}</div>}
      ...
      <section className="panel maintenance-evidence-panel">
        <header className="panel-header"><div><strong>Housekeeping</strong><span>Whether a reboot is waiting, services are healthy, the package lists are current, and security updates install themselves</span></div><span className={`status-pill ${maintenanceStatus === "healthy" ? "status-good" : maintenanceStatus === "review" ? "status-neutral" : "status-warning"}`}>{maintenanceStatus}</span></header>
        <div className="maintenance-evidence-summary">
          ...
          <div><span className="eyebrow">Reboot</span><strong>{maintenance.reboot.required === true ? "Required" : maintenance.reboot.required === false ? "Not required" : "Unavailable"}</strong><small>Ubuntu asked for a reboot after the last update</small></div>
          ...
          <div><span className="eyebrow">Security updates</span><strong>{maintenance.automaticSecurityUpdates.state.replaceAll("-", " ")}</strong><small>Whether Ubuntu installs security updates on its own</small></div>
        </div>
      </section>
      <section className="panel power-evidence-panel">
        <header className="panel-header"><div><strong>UPS power protection</strong><span>Battery backup status, read from the UPS plugged into this server</span></div><span className={`status-pill ${upsStatus === "healthy" ? "status-good" : upsStatus === "critical" || upsStatus === "warning" ? "status-warning" : "status-neutral"}`}>{upsStatus}</span></header>
