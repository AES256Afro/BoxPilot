...
export default function HostOverview() {
  ...
  const filesystems = inventory.storage.filesystems ?? { available: false, namespace: "unavailable" as const, mounts: [], summary: { healthy: 0, warning: 0, critical: 0, unavailable: 0 } };
  ...
  return (
    <>
      ...
      <section className="panel storage-evidence-panel">
        <header className="panel-header"><div><strong>Storage and filesystem evidence</strong><span>Host PID 1 mounts, sanitized block topology, and timer-generated SMART results</span></div><span className={`status-pill ${smart.status === "healthy" ? "status-good" : smart.status === "critical" || smart.status === "warning" ? "status-warning" : "status-neutral"}`}>{smart.status}</span></header>
        ...
        <div className="storage-boundary"><strong>Read-only evidence boundary</strong><span>The browser cannot select a device or run smartctl. A fixed root-only timer reads the host PID 1 mount table and writes bounded health fields; service-sandbox mounts, serials, UUIDs, raw SMART output, mount option values, and private home paths are excluded.</span></div>
      </section>
