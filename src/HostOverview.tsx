  ...
  return (
    <>
      ...
      <div className="dashboard-grid">
        ...
        <section className="panel"><header className="panel-header"><strong>Selected services</strong><span>Loaded units on this server</span></header><div className="workload-list">{inventory.services.filter((service) => service.load !== "not-found").map((service) => <div className="workload" key={service.unit}><div><strong>{service.unit}</strong><span>{service.sub} | {service.enabled}</span></div><span className="workload-kind">systemd</span><span className={`status-pill ${service.active === "active" ? "status-good" : "status-warning"}`}>{service.active}</span></div>)}</div></section>
      </div>
