...
describe("Backup Center", () => {
  it("plans and stages a restore-verified backup", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === "/api/v1/backups" && !init?.method) return new Response(JSON.stringify({ coverage: [{ applicationId: "uptime-kuma", name: "Uptime Kuma", source: { installed: true, healthy: true, state: "running", detail: "Managed container is running" }, state: "unprotected", protected: false, latestBackup: null, requirement: "Restore required" }], backups: [], limitations: ["Local only"] }), { status: 200 });
      if (url.endsWith("/plans") && init?.method === "POST") return new Response(JSON.stringify({ plan: { id: "plan-one", revision: "rev-one", output: { executable: true, destination: "local-managed", blockers: [], changes: ["Archive data"], warnings: ["Brief downtime"], recovery: "Restart source" } } }), { status: 201 });
    ...
    fireEvent.click(await screen.findByRole("button", { name: "Plan verified backup" }));
