...
describe("Tailscale panel", () => {
  it("offers exit node and subnet router and stages tailscale.set", () => {
    ...
    render(<TailscalePanel start={start} tailscale={{ connected: true, dnsName: "bigbox.tail1234.ts.net", address: "100.64.0.5", exitNodeAdvertised: false, advertisedRoutes: [], approvedRoutes: [], lanSubnets: ["192.168.1.0/24"] }} />);
  ...
  it("shows what is offered and what still needs approval, and disables the toggles when offline", () => {
    render(<TailscalePanel start={vi.fn()} tailscale={{ connected: true, dnsName: "bigbox.tail1234.ts.net", exitNodeAdvertised: true, advertisedRoutes: ["192.168.1.0/24"], approvedRoutes: [], lanSubnets: ["192.168.1.0/24"] }} />);
