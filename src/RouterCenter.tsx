...
export default function RouterCenter({ csrfToken }: { csrfToken: string }) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      setStatus(await readJson<RouterStatus>(await fetch("/api/v1/network/router-checkpoints")));
      setError(null);
    } catch (caught) {
  ...
  if (!status && loading) return <section className="vm-loading">Loading router checkpoint evidence...</section>;
