      ...
      <div className="app-grid">
        {applications.map((application) => (
          <article className="app-card" key={application.id}>
            ...
            {application.live.secretRetrievalCommand && application.live.installed && <span className="app-live-detail">Password from the server terminal: <code>{application.live.secretRetrievalCommand}</code></span>}
          ...
          <section className="modal app-plan-dialog" role="dialog" aria-modal="true" aria-labelledby="app-plan-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            {selected.id === "keel" && <>
              ...
              <div className="keel-artifact-proof">
                ...
                {selected.live.installation?.installed && <><span className="good-text">Private access: from your computer run <code>ssh -N -L 3000:127.0.0.1:3000 &lt;user&gt;@&lt;host&gt;</code>, then open <code>http://127.0.0.1:3000</code>.</span><span>After registering, copy Keel's one-use claim token, SSH to the server as your normal administrator, and run <code>sudo -k /usr/local/bin/node /opt/boxpilot/scripts/boxpilot-keel-claim.mjs 'PASTE_TOKEN'</code>. The fixed handoff rechecks the install, asks for fresh sudo, drops to the keel account, and never sends the token to BoxPilot.</span></>}
              </div>
              <div className="keel-artifact-proof">
                ...
                {selected.live.installation?.installed && !selected.live.loginProof?.verified && <span>After claim, run <code>sudo -k /usr/local/bin/node /opt/boxpilot/scripts/boxpilot-keel-owner-login-proof.mjs</code> from your normal server administrator account. Email and hidden password stay inside a short-lived unprivileged terminal worker. Security-key accounts are reported as incomplete and must be verified in the browser.</span>}
              </div>
              <div className="keel-artifact-proof">
            ...
            <div className="app-plan-fields">
              <label>Deployment target<select value={target} onChange={(event) => { setTarget(event.target.value); setPlan(null); }}>{selected.targets.map((item) => <option key={item} value={item}>{item === "virtual-machine" ? "Dedicated virtual machine" : item === "native-service" ? "Native service on this server" : "Docker on this server"}</option>)}</select></label>
