  ...
  return (
    <>
      ...
      <section className="panel backup-plan-card" aria-label="Controller disaster protection">
        <div className="section-heading"><div><span className="eyebrow">Independent controller destination</span><h3>{controllerDestination?.ready ? "Encrypted restic destination ready" : "Setup required on the server"}</h3></div><span className={`status-pill ${controllerDestination?.ready ? "status-good" : "status-warning"}`}>{controllerDestination?.ready ? "ready" : "blocked"}</span></div>
        ...
        {controllerDestination?.ready ? <p className="good-text">Mounted at {controllerDestination.mount?.target} on {controllerDestination.mount?.sourceType ?? "independent storage"}. Full repository reads and exact isolated restore drills are required.</p> : <div className="vm-plan-warnings"><strong>Fail-closed setup</strong>{controllerDestination?.blockers.map((blocker) => <span key={blocker}>{blocker}</span>)}<span>Run from the server terminal after mounting independent storage: <code>{controllerDestination?.setupCommand ?? "sudo /opt/boxpilot/scripts/boxpilot-controller-restic-setup.sh"}</code></span><span>Keep the controller repository password outside this server.</span></div>}
      </section>

      <section className="panel backup-plan-card" aria-label="Application disaster protection">
        <div className="section-heading"><div><span className="eyebrow">Independent application destination</span><h3>{applicationDestination?.ready ? "Encrypted restic destination ready" : "Setup required on the server"}</h3></div><span className={`status-pill ${applicationDestination?.ready ? "status-good" : "status-warning"}`}>{applicationDestination?.ready ? "ready" : "blocked"}</span></div>
        ...
        {applicationDestination?.ready ? <p className="good-text">Mounted at {applicationDestination.mount?.target} on {applicationDestination.mount?.sourceType ?? "independent storage"}. Full repository reads and exact byte-for-byte archive restores are required.</p> : <div className="vm-plan-warnings"><strong>Fail-closed setup</strong>{applicationDestination?.blockers.map((blocker) => <span key={blocker}>{blocker}</span>)}<span>Run from the server terminal after mounting independent storage: <code>{applicationDestination?.setupCommand ?? "sudo /opt/boxpilot/scripts/boxpilot-application-restic-setup.sh"}</code></span><span>Keep this recovery password separate from the controller and VM repository passwords.</span></div>}
      </section>
