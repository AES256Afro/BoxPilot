...
boxpilot_controller_device="$(stat -c %d /var/lib/boxpilot)"
boxpilot_local_backup_device="$(stat -c %d /var/lib/boxpilot-managed)"
if [ "$boxpilot_destination_device" = "$boxpilot_controller_device" ] || [ "$boxpilot_destination_device" = "$boxpilot_local_backup_device" ]; then
  printf 'The destination is on the same filesystem as BoxPilot state or local backups. It is not independent.\n' >&2
  exit 1
fi
...
if [ -L "$boxpilot_password_file" ]; then
  printf 'The controller restic password file cannot be a symbolic link.\n' >&2
  exit 1
fi
...
if [ ! -f "$boxpilot_password_file" ]; then
  ...
  umask 077
  boxpilot_temporary_password="$(mktemp "$boxpilot_secret_directory/controller-backup-restic-password.tmp.XXXXXX")"
...
if [ "$(stat -c %U:%G "$boxpilot_password_file")" != "root:root" ] || [ "$(stat -c %a "$boxpilot_password_file")" != "600" ] || [ "$boxpilot_password_size" -lt 16 ] || [ "$boxpilot_password_size" -gt 4096 ]; then
  printf 'The controller restic password file must be root:root mode 0600 and contain 16 to 4096 bytes.\n' >&2
  exit 1
fi
...
if [ -e "$boxpilot_repository/config" ]; then
  ...
  printf 'The encrypted controller restic repository is already initialized and readable.\n'
else
