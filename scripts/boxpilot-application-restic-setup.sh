...
boxpilot_mount="${BOXPILOT_APPLICATION_BACKUP_MOUNT:-/mnt/boxpilot-backup}"
boxpilot_repository="$boxpilot_mount/restic-applications"
boxpilot_secret_directory="/etc/boxpilot/secrets"
boxpilot_password_file="$boxpilot_secret_directory/application-backup-restic-password"
...
boxpilot_application_device="$(stat -c %d /var/lib/boxpilot-managed/apps)"
boxpilot_local_backup_device="$(stat -c %d /var/lib/boxpilot-managed/backups)"
if [ "$boxpilot_destination_device" = "$boxpilot_application_device" ] || [ "$boxpilot_destination_device" = "$boxpilot_local_backup_device" ]; then
  printf 'The destination is on the same filesystem as application state or local backups. It is not independent.\n' >&2
  exit 1
fi
...
if [ -L "$boxpilot_password_file" ]; then
  printf 'The application restic password file cannot be a symbolic link.\n' >&2
  exit 1
fi
...
if [ ! -f "$boxpilot_password_file" ]; then
  ...
  printf 'Create a separate application recovery password with at least 16 characters: '
  stty -echo
  ...
  umask 077
  boxpilot_temporary_password="$(mktemp "$boxpilot_secret_directory/application-backup-restic-password.tmp.XXXXXX")"
...
if [ "$(stat -c %U:%G "$boxpilot_password_file")" != "root:root" ] || [ "$(stat -c %a "$boxpilot_password_file")" != "600" ] || [ "$boxpilot_password_size" -lt 16 ] || [ "$boxpilot_password_size" -gt 4096 ]; then
  printf 'The application restic password file must be root:root mode 0600 and contain 16 to 4096 bytes.\n' >&2
  exit 1
fi
...
if [ -e "$boxpilot_repository/config" ]; then
  ...
  printf 'The encrypted application restic repository is already initialized and readable.\n'
else
  restic --repo "$boxpilot_repository" --password-file "$boxpilot_password_file" init
fi

printf 'Store a separate recovery copy of this application repository password outside this server.\n'
