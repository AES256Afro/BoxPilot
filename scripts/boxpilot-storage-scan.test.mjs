...
describe("fixed root-only storage scan", () => {
  ...
  it("discovers fixed disk names and never accepts a browser path", async () => {
    ...
    expect(result).toMatchObject({ available: true, disks: [{ device: "/dev/nvme0n1", health: "critical" }], boundary: { mutationPerformed: false, browserTriggered: false } });
  ...
  it("writes fail-closed evidence when smartctl is not installed", async () => {
    ...
    const scanner = createStorageScanner({ checkAccess: vi.fn(async () => { throw new Error("missing path secret"); }), now: () => new Date("2026-08-16T05:00:00.000Z") });
