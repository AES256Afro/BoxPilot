...
export const inspections = {
  ...
  "housekeeping.inspect": (() => {
    const categories = [
      ...
      { id: "restore-leftovers", title: "Unfinished restores", summary: "Staging or previous-data folders beside an app. They may contain an active restore or the only recoverable original.", items: 1, bytes: 256 * 1024 ** 2, detail: ["jellyfin: jellyfin.replaced"], keeping: ["jellyfin.replaced"], safe: false, unavailable: "Recovery evidence. General cleanup cannot remove these folders." },
    ...
    const totalBytes = categories.filter((category) => category.safe).reduce((sum, category) => sum + category.bytes, 0);
...
// running away with the drive while the library it feeds has barely moved.
  ...
  { appId: "qbittorrent", path: "/srv/media/torrents", mount: "/", bytes: 1340 * 1024 ** 3, grewBytes: 246 * 1024 ** 3, days: 7, sharedWith: ["qbittorrent", "sonarr"] },
  { appId: "sonarr", path: "/srv/media/torrents", mount: "/", bytes: 1340 * 1024 ** 3, grewBytes: 246 * 1024 ** 3, days: 7, sharedWith: ["qbittorrent", "sonarr"] },
...
], lastMeasured: { at: ago(9), sampled: 4, unmeasured: 0, error: null } }));   // ago() is hours: last night's sweep
